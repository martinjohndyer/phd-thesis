...
In this chapter I outline my work creating simulations of the GOTO observatory. \aref{sec:scheduler_sims} describes early simulations carried out when developing the G-TeCS scheduler in order to find the optimal ``tiebreak'' parameters, \aref{sec:multi_site} details how this simulation code could be modified to simulate multiple observing sites and \aref{sec:gw_sims} and \aref{sec:survey_sims} apply this to multi-site simulations of gravitational wave events and the all-sky survey respectively. All work described in this chapter is my own and has not been published before anywhere else.
...
\begin{itemize}
    ...
    \item Finally the fake pilot can be modified to simulate multiple observing telescopes at different sites, each of which in reality would use their own pilot. This is described in detail in \aref{sec:multi_site}.
\end{itemize}
