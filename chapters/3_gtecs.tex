...
Each daemon is built around a Python class which contains hardware control functions and a main loop. The loop is set running in its own thread, and when a control function is called it issues the relevant command to the hardware. The daemons are created using the Python Remote Objects (\textsf{Pyro}) module\footnote{Pyro-website}. Each daemon is a \textsf{Pyro} server, any client script can then access its functions across the network using the associated server ID.\@ For ease of use each daemon has an associated control script, which allows commands run by a user in a terminal to be executed by the daemon. In robotic mode these commands will be issued by the pilot (see Sec~\ref{sec:pilot}). Commands follow a simple format, e.g. \texttt{foc~home} homes all connected focusers, \texttt{dome~close} closes the dome, \texttt{cam~image~2~60} will take a 60 second science exposure on camera 2 only.
