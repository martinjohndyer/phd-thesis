...
\label{sec:fli}
% describe the hardware (see commissioning)
% fli-api
%   includes fakes
% fli interfaces:
%   need for n u c s
%   communicate through pyro
%   transfer images as num py arrays, so pickle
%   "meta-daemons"??
% cam:
%   types of images (light/dark), SCIENCE/etc, binning, area and so on
...
%   autofocus is later
% exq:
%   talks to both cam and filt
%   exposure list system
%       can pause and un-pause
%   multi filters
%   normal way to operate
...
% quick-close button
% lockdown/autoclose (see conditions)
%   depends on mode
% paul's heartbeat box
% dehumidifier
%   don't want to run too much (dust in rotor, loses efficiency, needs power)
%   auto turns on and off
...
% SiTechEXE
%   sync
%   PointXP (in commissioning section?)
% version 1: sitech interface (like fli)
%   had to run on Windows
%       start, shutdown and kill from Linux
%   cyg win and so on
% version 2: TCP/IP
%   much better!
%   commands with status info
% slew commands
%   target (distance from)
%   alt az
%   J now vs J 2000 (cooking)
% step commands
%   on-sky or RA (see issue)
% park/un-park
% "blinky" mode
% dec freezing
\begin{colsection}
...
\subsection{Power}
\label{sec:power}
% simplest of the daemons?
% lots of hardware
%   PDUs in rack
%   UPSs
%   relays on arms
% uses whatever that command system is called
% units defined in params
%   grouping outlets
\begin{colsection}
