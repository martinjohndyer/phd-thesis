...
In order to determine if GOTO is performing to expectations it is necessary to compare the theoretical zeropoints calculated in the previous section to real on-sky observations. To do this a set of sample observations were used: three images in all four filters of the Virgo Cluster taken on the 16th of March 2019. These images were taken during dark time and when the target was at a high altitude (airmass 1.08) to get the best quality data. Since GOTO is a very wide-field survey instrument there was no need to observe a particular standard star --- each frame contained thousands of individual sources that could be matched to a reference catalogue. The images were processed using the standard GOTO-photo pipeline (\todo{described in intro}), which calibrated the frames and extracted sources using Source Extractor \citep{SE}, which used the gain values for each camera calculated in \aref{sec:ptc}. The sources were then matched to the Pan-STARRS PS1 catalogue \citep{Pan-STARRS}, and the catalogue magnitudes were plotted against the background-subtracted instrumental magnitudes from Source Extractor. A linear fit was made against this plot, and the intercept was taken as the zeropoint for that image. The zeropoints for each image are given in \aref{tab:real_zeropoints}. The best for each set of three were selected and compared to the theoretical AB magnitudes calculated using pysynphot in \aref{tab:zeropoint_comparison}. For each filter and unit telescope a performance factor is calculated based on the ratio of observed to theoretical signals.

In most cases the observed zeropoints are very close to the theoretical model values. Further images taken over more nights and other targets would be needed to make any firm conclusions on potential hardware problems, but a few trends are clear. UT3 is consistently better performing than the others, which is expected since it was the last mirror to be reground just a few months before the images were taken and therefore would be clear of any dust or imperfections the other mirrors have built up. On the other hand UT1 is consistently the worst performing of the four which is harder to explain. It's also clear that the throughput in the \textit{G} band has somehow been underestimated, as the real observations are consistently recording higher signals than expected.
