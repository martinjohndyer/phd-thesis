...
The shot noise ($\sigma_\text{SN}$) arises as photons from the target object arrive at the sensor at irregular intervals. The photon arrival time is a Poisson distribution, and if the number of electrons counted is $N$ for large numbers it tends towards a Gaussian distribution with mean $N$ and standard deviation $\sigma_\text{SN} = \sqrt{N}$. When taking on-sky astronomical observations there are two sources of shot noise, from the target object ($\sigma_\text{O}$) and the background sky ($\sigma_\text{BG}$), but the latter is not relevant for the in-lab tests described in this section.

Dark current noise ($\sigma_\text{D}$) is due to electrons produced by thermal excitations, which are indistinguishable from photo-electrons and increase with exposure time. As this is also a photon counting measurement it follows a Poisson distribution, so the noise $\sigma_\text{D} = \sqrt{D}$ where $D$ is the dark current per pixel. The dark current depends on temperature: cooling the cameras reduces the thermal excitations and therefore reduces the dark current.
...
\aref{eq:ptc} is a quadratic equation in $S$ which relates the measured total signal $S$ to the variance in the signal $\sigma_S^2$, and can be fitted to a photon transfer curve to determine values for the gain $g$, read-out noise $R$ and fixed-pattern noise $k_\text{FP}$. The key features of a photon transfer curve are common for all CCDs, and are shown in cartoon form in \aref{fig:ptc_cartoon}. The first noise regime is when the signal is small; for small $S$ in \aref{eq:ptc} the noise is constant, and as there is no signal the overall noise is limited by the detector noise. This will include both the read-out noise and the dark current, but for the very short exposure times used (less than 1 second) the dark current is negligible and the read-out noise dominates. At higher signals the noise is dominated by the shot noise which is proportional to the $\sqrt{S}$, so this region has a gradient of 1/2 when plotted on the log-log axis and value of the gain is where the linear fit crosses the x-axis (when $S=0$). As the signal increases further the fixed-pattern noise begins to dominate, and, as this noise is proportional to the signal, this produces a gradient of 1 in the PTC.\@ Finally, the pixel reaches its full well capacity (assuming the gain has been set so this occurs before the ADU saturates), so the noise drops to zero.
    ...
    \begin{center}
        \begin{tabular}{l|cc|cc|cc|cc} %chktex 44
            ...
            \multicolumn{2}{c}{(ADU)} \\
             & L & R & L & R & L & R & L & R \\
            \midrule
            Camera 1 & 0.53 & 0.53 & 12.4 & 12.0 & 0.24 & 0.24 & 64568 & 64585 \\
            Camera 2 & 0.53 & 0.53 & 11.9 & 11.7 & 0.23 & 0.24 & 64552 & 64555 \\
            Camera 3 & 0.57 & 0.57 & 12.6 & 11.8 & 0.26 & 0.24 & 64540 & 64552 \\
            Camera 4 & 0.57 & 0.58 & 13.4 & 14.0 & 0.24 & 0.25 & 64577 & 64536 \\
            Camera 5 & 0.62 & 0.63 & 12.3 & 12.8 & 0.25 & 0.25 & 64544 & 64550 \\
            Camera 6 & 0.63 & 0.62 & 11.8 & 12.6 & 0.25 & 0.25 & 64554 & 64545 \\
            Camera 7 & 0.62 & 0.62 & 13.1 & 12.5 & 0.25 & 0.24 & 64544 & 64552 \\
            Camera 8 & 0.62 & 0.62 & 14.3 & 12.2 & 0.26 & 0.25 & 64529 & 64522 \\
        \end{tabular}
    \end{center}
...
The gain values are all around 0.6 \elec/ADU, and would have been set as such to maximise the dynamic range based on the full well capacity. The saturation levels are all around 64550, and 64550 $\times$ 0.6 $\approx$ 39,000 \elec, which is matches the advertised 40,300 capacity. The read-out noise values also match the FLI specifications, which advertise a typical read-out noise of 12 \elec{} for the MicroLine cameras. Finally, the fixed-pattern noise is a low fraction of the signal. This was to be expected, as modern CCD sensors have very low pixel non-uniformities.
...
Dark current noise is independent of the incoming signal but depends on the exposure time of the image. It also increases as a function of temperature, $T$. The dark current per second increases at an exponential rate and can be modelled using the equation
%
\begin{equation}
    D(T) = Ae^{kT + C},
    \label{eq:dark_model}
...
\begin{figure}[p]
    ...
    \caption[An example of a column defect]{
        A flat field for Camera 1 showing a bad column. The location of the trap pixel is magnified, the pixels in the column above the trap have been prevented from being read out. The bad column is also also clearly visible in the plot below showing counts averaged in the y axis.
    }\label{fig:itsatrap}
\end{figure}
...
Unfortunately, not every photon emitted by a target object will be recorded by a telescope. Photons will be lost due to absorption and scattering in the Earth's atmosphere, and the telescope's optics and camera. Understanding each of these factors is required in order to produce a complete throughput model, which can then be compared to the real system (in \aref{sec:onsky_comparison}) to see if the hardware is performing to the expected level.
