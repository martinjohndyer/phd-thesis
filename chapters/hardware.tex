...
\begin{figure}[t]
    ...
    \caption[Complete throughput model for the GOTO filters]{
        The complete GOTO throughput model. The model elements (dashed lines) are the throughput of the OTA elements (from \aref{fig:trans_ota}) and the quantum efficiency of the microlensed CCD (from \aref{fig:qe}); and the Baader \textit{LRGB} filter bandpasses (from \aref{fig:filters}) are shown by the coloured dotted lines. The filled lines shown the total throughput in each filter when the model is applied.
...
Finally, multiplying the theoretical fluxes in \aref{tab:zeropoints} by the mean throughput values for each filter from \aref{tab:throughput_extinction} gives the predicted signal in the detector in photo-electrons per second. Converting these signals into magnitudes using \aref{eq:ins_mag} gives the instrumental magnitude, and using \aref{eq:zp2} with $m=0$ gives the theoretical zeropoint. The predicted signals and zeropoints are also given in \aref{tab:zeropoints}.
