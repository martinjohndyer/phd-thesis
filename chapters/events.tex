...
At the core of the GOTO-alert code is the \code{Event} object class. An Event is created from a raw binary payload from the PyGCN VOEvent client, and from the \gls{xml} a \proglang{Python} class is populated with the basic event information (\gls{ivorn}, type, source etc). Once the basic Event is created it is checked against an internal list of so-called ``interesting'' event packet types --- the ones we care about processing for \gls{goto}. At the time of writing these are \code{SWIFT\_BAT}, \code{FERMI\_GBM} and \code{LVC} events, as listed in Table~\ref{tab:events}. If the event matches any of the recognised packet types then the Event is subclassed into a new object, which allows more specific properties and methods. The current classes are as follows:
...
\midrule
\code{3x60L}   &          1/1 &                   3 & \SI{60}{\second} &      L \\ % chktex 29
\code{3x60RGB} &          1/3 &                   1 & \SI{60}{\second} &      R \\ % chktex 29
                &          2/3 &                   1 & \SI{60}{\second} &      G \\ % chktex 29
                &          3/3 &                   1 & \SI{60}{\second} &      B \\ % chktex 29
...
\rtxt{How does this fit into the scheduling sections? What do the readers know at this point about ranks or constraints? Wouldn't this be better after the scheduling chapter (or even within it)?}
...
Once previous event have been dealt with then the new entries in the database need to be created. All \gls{goto} pointings from the sentinel are defined \textit{on-grid}, meaning that they need to be mapped to the current GOTO-tile sky grid (see Section~\ref{sec:grids}). The database \code{Grids} table contains the field of view and overlap parameters of the current grid as well as the algorithm used (see Section~\ref{sec:algorithms}), allowing it to be reconstructed using GOTO-tile within the event handler. Once a GOTO-tile \code{SkyGrid} class has been created then a corresponding \code{SkyMap} class is made based on the information in the event. If it was created from a gravitational wave alert then it should have a URL to download the \gls{lvc}-created sky map. If instead it just has a coordinate and error radius, i.e.\ a \gls{grb} alert, then a new Gaussian sky map is constructed as described in Section~\ref{sec:gaussian_skymaps}. Once both grid and map are ready then the sky map is mapped onto the grid simply using the class method \code{SkyGrid.apply\_skymap(SkyMap)}, which returns a table of tiles and associated contained probabilities (as described in Section~\ref{sec:mapping_skymaps}). %chktex 36
...
\caption[An example 2D skymap with pixel contour values]{The same cartoon skymap as in Figure~\ref{fig:sim_skymap_probs}, but now each pixel contains its contour value. These are calculated for each pixel by sorting by probability and assigning each the value of the cumulative sum. This means pixel E5, which has the highest single probability, keeps the same value while the second highest (F5) has the sum of both E5 and F5 and so on.\\
The thick blue line is the 50\% probability contour, which encloses all pixels with a contour value of less than 50\% (so pixel G3 is just within the 50\% area but pixel B4 is outside). The green line is the 90\% contour area and likewise encloses all pixels with a contour value of less than 90\%.
}
...
\begin{center}
\begin{tabular}{ccc}
Tile minimum contours &
Tiles within the &
Tiles within the
\\
(from Figure~\ref{fig:sim_skymap_tiles}): &
50\% contour: &
90\% contour:
\\
\includegraphics[width=0.3\linewidth]{images/sim/sim_skymap_tile_minconts.pdf} &
\includegraphics[width=0.3\linewidth]{images/sim/sim_skymap_tiles_a.pdf} &
\includegraphics[width=0.3\linewidth]{images/sim/sim_skymap_tiles_b.pdf}
\\
\\
Tile mean contours &
Tiles with &
...
\\
(from Figure~\ref{fig:sim_skymap_tiles}): &
mean contour value < 50\%: &
mean contour value < 90\%:
\\
\includegraphics[width=0.3\linewidth]{images/sim/sim_skymap_tile_meanconts.pdf} &
\includegraphics[width=0.3\linewidth]{images/sim/sim_skymap_tiles_c.pdf} &
\includegraphics[width=0.3\linewidth]{images/sim/sim_skymap_tiles_d.pdf}
\\
\end{tabular}
\end{center}

\caption[Selecting tiles based on contour levels for a 2D skymap]{Selecting tiles for the same cartoon skymap as in Figure~\ref{fig:sim_skymap_tiles}.\\
\textbf{Top}: These three plots show selecting tiles based on calculating the tile contour value by selecting the minimum contour value of the 9 pixels within the tile. This method can lead to a single pixel having a disproportionate influence: for example the top right tile is included within the green 50\% contour based on the presence of a single pixel that only just makes the cut-off. With real high-resolution skymaps a single tile will contain a much higher number of pixels, and yet using this method the tile could still be selected based on a single outlier pixel.\\
\textbf{Bottom}: These three plots instead show selecting tiles based on calculating the tile contour value by taking the \textit{mean} contour value of the 9 contained pixels. This is a more selective method that results in tiles being affected less by single outliers.

}
\label{fig:sim_skymap_tiling}
\end{figure}
