...
The tile probability table created by GOTO-tile contains entries for every tile within the all-sky grid, of which a vast majority will contain a very small amount of the overall probability for any reasonably-well located sky map. Adding an entry to for every tile would therefore be unnecessary, and even harmful as the scheduling system is designed to complete a full pass of the visible tiles before going on to re-observe those already completed. Therefore it is necessary to prune the tile table before inserting tiles into the database.

There are multiple ways to chose the tiles to insert. Initially there was a simple cut-off in terms of each tile's contained probability: insert each tile that contains greater than $1\%$ of the total probability ($5\%$ for \glspl{grb}). This hard limit however proved to be unsuitable. \gls{gw} skymaps cover a large range in sky area, and for very well spread-out skymaps there might be few if any tiles that reach that limit. On the other hand for very well localised events adding tiles down to the $1\%$ level is redundant and would waste time if there are tiles that contain tens of percent. Therefore a new method of selecting tiles had to be found.
...
\\
\multicolumn{2}{c}{\includegraphics[width=0.6\linewidth]{images/tiling/1_0.png}}
\\
...
\\
\multicolumn{2}{c}{\includegraphics[width=0.6\linewidth]{images/tiling/2_0.png}}
\\
Tiles within 50\% contour: &
Tiles within 90\% contour:
\\
...
\\
\multicolumn{2}{c}{\includegraphics[width=0.6\linewidth]{images/tiling/3_0.png}}
\\
Tiles within 50\% contour: &
Tiles within 90\% contour:
\\
