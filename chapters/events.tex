...
The thick blue line is the 50\% probability contour, which encloses all pixels with a contour value of less than 50\% (so pixel G6 is just within the 50\% area but pixel B4 is just outside). The green line is the 90\% contour area and likewise encloses all pixels with a contour value of less than 90\%.
}
...
\end{tabular}
\end{center}
\caption[Calculating tile contained probabilities and contours]{Tiling the same cartoon 2D skymap as Figures~\ref{fig:sim_skymap_probs}~\&~\ref{fig:sim_skymap_conts}.\\
...
\textbf{Lower-centre}: One way of finding which contour area each tile is within is to find the minimum contour value of the (in this example 9) pixels contained within it. This corresponds to the contour value of the pixel with the highest probability within each tile.\\
\textbf{Lower-right:} An alternative method is to instead calculate the mean contour value of the pixels within each tile.
}
\label{fig:tiling_S190425z}
\end{figure}
